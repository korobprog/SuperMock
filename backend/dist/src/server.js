...
try {
    ...
    console.log(`.env файл в корневой директории: ${envExists ? 'найден' : 'не найден'}`);
    if (envExists) {
...
alternativePaths.forEach((path) => {
    ...
    if (exists) {
        try {
            ...
            console.log(`  Содержимое (${files.length} файлов): ${files
                .slice(0, 5)
                .join(', ')}${files.length > 5 ? '...' : ''}`);
            ...
            console.log(`  Ошибка при чтении директории: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
});
if (frontendExists) {
    console.log('Директория с фронтендом найдена. Настраиваем обслуживание статических файлов.');
    try {
        ...
        console.log(`Ошибка при чтении директории фронтенда: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    console.log('Директория с фронтендом не найдена. Сервер будет обслуживать только API.');
    ...
    const reactFrontendPath = path_1.default.join(__dirname, '../../react-frontend');
    ...
    if (reactFrontendExists) {
        try {
            ...
            console.log(`Содержимое директории react-frontend: ${files
                .slice(0, 10)
                .join(', ')}${files.length > 10 ? '...' : ''}`);
        }
        catch (error) {
            console.log(`Ошибка при чтении директории react-frontend: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    // Маршрут для всех остальных запросов, когда фронтенд отсутствует
    app.get('*', (req, res) => {
        res.status(404).json({
            message: 'Фронтенд не найден на сервере. Используйте API-эндпоинты или перейдите на https://supermock.netlify.app',
        });
    });
}
// Обработка ошибок
...
server.listen(PORT, () => {
    ...
    console.log('Также используется дополнительный middleware CORS для гибкой настройки');
