...
router.get('/', auth_1.auth, async (req, res) => {
    try {
        ...
        const calendarEntries = await InMemoryCalendar_1.InMemoryCalendarEntry.find();
        ...
        console.error('Ошибка при получении списка записей календаря:', error.message);
        res.status(500).json({ message: 'Ошибка сервера' });
    }
});
// Получение записи календаря по ID сессии
// GET /api/calendar/session/:sessionId
router.get('/session/:sessionId', auth_1.auth, async (req, res) => {
    try {
        ...
        console.error('Ошибка при получении записи календаря:', error.message);
        res.status(500).json({ message: 'Ошибка сервера' });
    }
});
...
router.post('/', auth_1.auth, async (req, res) => {
    try {
        ...
        (0, websocket_1.notifyCalendarUpdated)(req.app.get('io'), {
        ...
        console.error('Ошибка при создании записи календаря:', error.message);
        res.status(500).json({ message: 'Ошибка сервера' });
    }
});
// Обновление записи календаря
// PUT /api/calendar/session/:sessionId
router.put('/session/:sessionId', auth_1.auth, async (req, res) => {
    try {
        const { sessionId } = req.params;
        ...
        console.error('Ошибка при обновлении записи календаря:', error.message);
        res.status(500).json({ message: 'Ошибка сервера' });
    }
});
