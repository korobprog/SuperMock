...
const applicationDistributionService_1 = require("../services/applicationDistributionService");
    ...
    try {
        ...
        if (comments && comments.trim().length > 10) {
            try {
                ...
                const aiService = new aiAnalysisService_1.AIAnalysisService();
                // Используем setImmediate чтобы не блокировать ответ пользователю
                setImmediate(async () => {
                    try {
                        // Анализируем фидбек (включая ratings и comments)
                        const analysis = await aiService.analyzeFeedback(parseInt(feedback.id) || 0, comments, ratings || {}, // 📊 Теперь передаем рейтинги!
                        profession, userLanguage, targetUserId);
                        ...
                        try {
                            const distributionService = new applicationDistributionService_1.ApplicationDistributionService();
                            const distributedContent = await distributionService.distributeRecommendations(analysis, recommendations, targetUserId, profession, userLanguage);
...
router.get('/:id/analysis', auth_1.auth, async (req, res) => {
    try {
...
router.get('/users/:id/skill-analysis', auth_1.auth, async (req, res) => {
    try {
        const targetUserId = req.params.id;
        const requestUserId = req.user?.id;
        if (!requestUserId) {
...
router.get('/users/:id/recommendations', auth_1.auth, async (req, res) => {
    try {
        const targetUserId = req.params.id;
        const requestUserId = req.user?.id;
        if (!requestUserId) {
...
router.get('/users/:id/ai-materials', auth_1.auth, async (req, res) => {
    try {
        const targetUserId = req.params.id;
        const requestUserId = req.user?.id;
        if (!requestUserId) {
...
router.get('/users/:id/ai-roadmap', auth_1.auth, async (req, res) => {
    try {
        const targetUserId = req.params.id;
        const requestUserId = req.user?.id;
        if (!requestUserId) {
        ...
        if (skill) {
            filteredStages = filteredStages.filter(s => s.skills.includes(skill));
        }
        res.json({
...
router.get('/users/:id/ai-calendar', auth_1.auth, async (req, res) => {
    try {
        const targetUserId = req.params.id;
        const requestUserId = req.user?.id;
        if (!requestUserId) {
...
router.get('/users/:id/ai-training-tasks', auth_1.auth, async (req, res) => {
    try {
        const targetUserId = req.params.id;
        const requestUserId = req.user?.id;
        if (!requestUserId) {
