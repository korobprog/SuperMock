...
import telegramAuthRouter from '../src/routes/telegram-auth.js';
...
async function createMatchesFromQueues() {
  try {
    ...
    // Find distinct combinations where both roles are waiting using DB adapter
    const waiting = await dbAdapter.findUserQueues({ status: 'waiting' });
...
app.get('/api/history/:userId', async (req, res) => {
  ...
  try {
    ...
    // Handle deployments where feedback table may not have new columns yet
    let feedbacks = [];
    try {
      ...
      console.warn('Feedback query failed, returning empty feedbacks:', e?.code || e?.message);
      feedbacks = [];
    }
...
app.get('/api/user-data-check/:userId', async (req, res) => {
  const { userId } = req.params;
  const professionFilter = req.query.profession
    ? String(req.query.profession)
    : undefined;

  try {
    // Check if user exists using direct DB adapter
    const user = await dbAdapter.findUserById(String(userId));
    ...
    const tools = await prisma.userTool.findMany({
      where: {
        userId: String(userId),
        ...(targetProfession ? { profession: targetProfession } : {}),
      },
      select: { toolName: true },
    });
