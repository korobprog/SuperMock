...
router.post('/:id/roles', auth, async (req, res) => {
  try {
    ...
    // автоматически генерируем ссылку через Google Meet API
    if (
      role === 'interviewer' &&
  ...
  try {
    ...
    if (manualLink) {
      ...
      try {
        ...
        if (!validationResult.isValid) {
          return res.status(400).json({
            message: `Некорректная ссылка на Google Meet: ${validationResult.message}`,
          });
        }
        ...
        console.log(
          `Ручная ссылка на Google Meet успешно проверена: ${manualLink}`
        );
      } catch (error) {
        console.error('Ошибка при проверке ссылки на Google Meet:', error);
        return res.status(500).json({
          message: 'Ошибка при проверке ссылки на Google Meet',
          details: error.message,
        });
      }
    }
      ...
      try {
        ...
        if (error.code) {
          ...
          if (error.code === 'ENOTFOUND') {
            clientErrorMessage +=
              ': проблема с подключением к серверу Google Meet';
          } else if (error.code === 'ECONNREFUSED') {
            clientErrorMessage += ': сервер Google Meet отклонил соединение';
          } else if (error.code === 'ETIMEDOUT') {
            clientErrorMessage +=
              ': превышено время ожидания ответа от сервера Google Meet';
          } else {
          ...
          if (error.response.status === 401) {
            clientErrorMessage += ': ошибка авторизации в Google Meet API';
          } else if (error.response.status === 403) {
            clientErrorMessage += ': отказано в доступе к Google Meet API';
          } else if (error.response.status === 404) {
            clientErrorMessage += ': ресурс Google Meet API не найден';
          } else if (error.response.status === 429) {
            clientErrorMessage += ': превышен лимит запросов к Google Meet API';
          } else {
...
router.get('/:id/video', auth, async (req, res) => {
  try {
    ...
    // Проверяем статус ссылки через Google Meet API (если API поддерживает)
    if (session.videoLink && session.videoLinkStatus === 'active') {
