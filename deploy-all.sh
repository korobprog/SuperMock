...
# Скрипт для полного деплоя приложения на сервер
# Останавливает контейнеры, удаляет старые образы и устанавливает новые
...
docker build -t $DOCKER_USERNAME/web-app-frontend:latest ./react-frontend --no-cache
...
docker build -t $DOCKER_USERNAME/web-app-backend:latest ./backend --no-cache
...
scp docker-compose.prod.yml .env nginx/nginx.conf $SERVER_USER@$SERVER_HOST:$SERVER_PATH/

if [ $? -ne 0 ]; then
  error "Ошибка при копировании файлов на сервере."
fi
