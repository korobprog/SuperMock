...
if [ -f "frontend/src/components/ui/user-tools-display.tsx" ]; then
    if grep -q "getDemoToolsForProfession" "frontend/src/components/ui/user-tools-display.tsx"; then
        echo "   ✅ user-tools-display.tsx содержит fallback"
    else
        echo "   ❌ user-tools-display.tsx не содержит fallback"
    fi
else
...
echo "Откройте test-fallback.html в браузере для проверки fallback логики"
